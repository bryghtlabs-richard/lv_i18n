...
${Object.entries(data[l].plural[form]).sort().map(([ k, val ]) => `    {"${esc(k)}", "${esc(val)}"},`).join('\n')}
};
`.trim();
}
...
${Object.entries(data[l].singular).sort().map(([ k, val ]) => `    {"${esc(k)}", "${esc(val)}"},`).join('\n')}
};
`.trim();
}
...
${pforms.map(pf => `    .num_plurals[${pf_enum[pf]}] = ${Object.keys(data[l].plural[pf]).length},`).join('\n')}
    .num_singulars = ${Object.keys(data[l].singular).length},
